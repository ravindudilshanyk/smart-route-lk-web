...
router.get("/today", async (req, res) => {
  ...
  try {
    // Find which bus this conductor is assigned to
    ...
    const passRes = await pool.query(
      `SELECT bp.passenger_name, bp.seat_number, bp.boarded, bp.no_show,
              bp.gender, bp.qr_token,
...
router.post("/scan", async (req, res) => {
  ...
  try {
    ...
    if (conductorRes.rows.length === 0) {
      return res.status(403).json({ error: "No bus assigned." });
    }

    const bus_id = conductorRes.rows[0].bus_id;

    // Find passenger by QR token
    const passengerRes = await pool.query(
      `SELECT bp.*, bk.bus_id, bk.board_stop_id, bk.drop_stop_id,
              bs1.stop_name AS board_stop,
              bs2.stop_name AS drop_stop
       FROM booking_passengers bp
       JOIN bookings bk ON bk.id = bp.booking_id
    ...
    if (passenger.bus_id !== bus_id) {
      return res.status(403).json({
        error: `This ticket is for a different bus. Valid for bus ID: ${passenger.bus_id}`,
      });
    }
